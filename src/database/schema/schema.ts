import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, unique, int, varchar, timestamp, foreignKey, float, text, date, datetime } from "drizzle-orm/mysql-core"
...
export const cinemas = mysqlTable("cinemas", {
	...
	name: varchar({ length: 255 }).notNull(),
	url: varchar({ length: 255 }).notNull(),
	filmwebCityId: int().notNull().references(() => cities.filmwebId),
	longitude: float(),
	latitude: float(),
...
export const movies_actors = mysqlTable("movies_actors", {
	id: int().autoincrement().notNull(),
	movieId: int().notNull().references(() => movies.id),
...
(table) => [
	...
	unique("movies_actors_movieId_actorId_unique").on(table.movieId, table.actorId),
]);

export const movies_countries = mysqlTable("movies_countries", {
	id: int().autoincrement().notNull(),
	movieId: int().notNull().references(() => movies.id),
	countryId: int().notNull().references(() => countries.id),
	createdAt: timestamp({ mode: 'string' }).default(sql`(now())`).notNull(),
	updatedAt: timestamp({ mode: 'string' }).default(sql`(now())`).notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "movies_countries_id"}),
	unique("movies_countries_movieId_countryId_unique").on(table.movieId, table.countryId),
]);

export const movies_directors = mysqlTable("movies_directors", {
	id: int().autoincrement().notNull(),
	movieId: int().notNull().references(() => movies.id),
	directorId: int().notNull().references(() => directors.id),
	createdAt: timestamp({ mode: 'string' }).default(sql`(now())`).notNull(),
	updatedAt: timestamp({ mode: 'string' }).default(sql`(now())`).notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "movies_directors_id"}),
	unique("movies_directors_movieId_directorId_unique").on(table.movieId, table.directorId),
]);

export const movies_genres = mysqlTable("movies_genres", {
	id: int().autoincrement().notNull(),
	movieId: int().notNull().references(() => movies.id),
...
(table) => [
	...
	unique("movies_genres_movieId_genreId_unique").on(table.movieId, table.genreId),
]);
...
export const screenings = mysqlTable("screenings", {
	...
	movieId: int().notNull().references(() => movies.id),
	showtimeId: int().notNull().references(() => showtimes.id),
	cinemaId: int().notNull().references(() => cinemas.filmwebId),
...
(table) => [
	primaryKey({ columns: [table.id], name: "screenings_id"}),
]);

export const showtimes = mysqlTable("showtimes", {
	id: int().autoincrement().notNull(),
	url: varchar({ length: 255 }).notNull(),
	cityId: int().notNull().references(() => cities.id),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
